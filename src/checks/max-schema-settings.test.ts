import { check } from '@shopify/theme-check-common/dist/test';
import { describe, expect, it } from 'vitest';
import { MaxSchemaSettings } from './max-schema-settings.js';
...
function makeSchema(settings: object[]): string {
  return ['<div></div>', '{% schema %}', JSON.stringify({ name: 'My section', settings }), '{% endschema %}'].join(
    '\n',
  );
}
...
describe('MaxSchemaSettings', () => {
  ...
  it('reports on block files', async () => {
    const settings = Array.from({ length: 6 }, (_, i) => makeSetting(`setting_${i}`));
    const source = ['{% schema %}', JSON.stringify({ name: 'My block', settings }), '{% endschema %}'].join('\n');
    const offenses = await check(
      { [blockFile]: source },
      [MaxSchemaSettings],
      {},
  ...
  it('does not report on non-section/block files', async () => {
    const settings = Array.from({ length: 6 }, (_, i) => makeSetting(`setting_${i}`));
    const source = ['{% schema %}', JSON.stringify({ name: 'Not a section', settings }), '{% endschema %}'].join('\n');
    const offenses = await check(
  ...
  it('does not report when schema has no settings', async () => {
    const source = ['<div></div>', '{% schema %}', JSON.stringify({ name: 'My section' }), '{% endschema %}'].join(
      '\n',
    );
    const offenses = await check(
      { [sectionFile]: source },
